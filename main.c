    ...
    while (!WindowShouldClose()) {
        ...
        for (int i = 0; i < NUM_BOTOES; i++) {
            if (CheckCollisionPointRec(mouse, botoes[i].rect) &&
                ...
                if (i == NUM_BOTOES - 1) CloseWindow();
                /* os outros casos serao implementados depois */
            }
        }
        ...
        BeginDrawing();
            ...
            for (int i = 0; i < NUM_BOTOES; i++) {
                ...
                DrawText(botoes[i].texto, (int)(botoes[i].rect.x + botoes[i].rect.width  / 2 - tw / 2),(int)(botoes[i].rect.y + botoes[i].rect.height / 2 - 10), 20, cor_texto);
            }

            /* Rodape */
            DrawText("Cauã | Bernardo | João Arthur | Arthur Reis", 185, 555, 16, COR_SUBTITULO);
